  ...
  try {
    await session.withTransaction(async () => {
      ...
      for (const escrow of expiredEscrows) {
          ...
          if (escrow.metadata?.quote_id) {
            ...
            if (quote) {
              ...
              try {
                ...
                console.error(
                  "Error sending quote cancellation email:",
                  emailError,
                );
              }
            }
        ...
        if (!creatorWallet) {
          console.error(
            `No wallet found for user ${escrow.creatorId._id} in currency ${escrow.currency}`,
          );
          continue;
        }
        ...
        // Cancel associated quote if it exists
        if (escrow.metadata?.quote_id) {
          const quote = await Quote.findById(escrow.metadata.quote_id).session(
            session,
          );
          if (quote) {
            quote.status = "Cancelled";
